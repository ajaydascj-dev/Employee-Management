...
const renderLog = (req, res) => {
  res.status(200).render("signupIn");
};
...
const signIn = async (req, res) => {
  ...
  if (!email || !password) {
    ...
    if (existingUser.length > 0) {
      const savedPassword = existingUser[0].password;
      ...
      if (verified) {
        ...
        let token = jwt.sign(resp, process.env.MY_SECRET, { expiresIn: "5h" });
      ...
      req.session.message = {
        type: "warning",
        info: "You dont have an account with this mail please sign up",
      };
      res.redirect("/user/login");
    }
  }
  ...
  if (userOtprecord.length > 0) {
    ...
    if (expiresAt > Date.now()) {
      ...
      if (verified) {
        ...
        req.session.message = {
          type: "success",
          info: "You are successfully registered please singn in",
        };
